...
function buildInitialRules(targetingRules) {
    if (!targetingRules || targetingRules.length === 0) return []
    return targetingRules.map(({ key, values, enabled }) => ({
        key: key || "",
        values: values || [],
...
function UpdateBox({ setOverlay, setBox, setChanged, flag, showToast }) {

    const [rules, setRules] = useState(() => buildInitialRules(flag.targetingRules))
    const [newValueInputs, setNewValueInputs] = useState(() => buildInitialRules(flag.targetingRules).map(() => ""))
    ...
    async function updateFlag(flag) {
        ...
        const response = await fetch(`/flags/${flag.id}`, {
            method: "PUT",
    ...
    return (
        <div className="modal-box">
            ...
            <form className="modal-form">
                ...
                <input type="text" defaultValue={flag.flagName} className="field-input" id="flagName" placeholder="my-feature" onChange={() => setInfo("")} />

                <label className="field-label">Description</label>
                <input type="text" defaultValue={flag.description} className="field-input" id="description" placeholder="What does this flag do?" />

                <label className="field-label">Rollout Percent</label>
                <input type="number" defaultValue={flag.rolloutPercent} className="field-input" id="rolloutPercent" min="0" max="100" placeholder="0–100" onChange={() => setInfo("")} />
                ...
                <div className="modal-footer">
                    <button type="button" className="btn-cancel" onClick={() => { setOverlay(false); setBox(false) }}>Cancel</button>
                    <button type="button" className="btn-submit" onClick={() => updateFlag(flag)}>Save Changes</button>
                </div>
            </form>
        </div>
    )
